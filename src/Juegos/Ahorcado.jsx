	...
	const printLetter = (letter) => {
		for (let i = 0; i < palabraaencontrar.length; i++) {
			if (letter == caracterespalabraaencontrar[i] && letrausada !== true) {
				...
				palabraescondida[i] = palabraescondida[i].replace(" - ", letter);
				acertadas[i] = letter;
		...
		if (vida === 0 || letraencontrada === palabraaencontrar.length) {
			...
			if (letraencontrada === palabraaencontrar.length) {
				setMensajePrincipal(
					`Felicitaciones, adivinaste la palabra escondida ${palabraaencontrar}!!!`
				);
				console.log("Ganaste");
			} else {
				setMensaje2(
					`Perdiste no te quedan mas vidas, la palabra a encontrar era ${palabraaencontrar}`
				);
			}
		}
	};
	...
	return (
		<>
			<div className="contenedor principal_section5">
				...
				<div>
					{" "}
					<p className="escondida">
						La palabra tiene {palabraaencontrar.length} letras
					</p>
				</div>

				<div className="escondida">{palabraescondida.join("")}</div>
